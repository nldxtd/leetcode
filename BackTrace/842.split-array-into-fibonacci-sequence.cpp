...
public:
    ...
    bool backtrack(string s, int start) { // helper function (perfom backtracking)
        if (start == s.size() and ans.size() > 2)
            return true; // if we trverse whole string and atleast there are are 3 fibonacci number then return true.
